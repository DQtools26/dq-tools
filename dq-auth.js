...
function buildAuthModal() {
  ...
  modal.innerHTML = `
    <div id="dq-auth-box">
      ...
      <button id="dq-google-btn" onclick="DQAuth.signInGoogle()">
...
window.DQAuth = {
  ...
  async signInGoogle() {
    if (!_auth) return;
    try {
      const { GoogleAuthProvider, signInWithPopup } = window._DQFire;
      const cred = await signInWithPopup(_auth, new GoogleAuthProvider());
      const uid = cred.user.uid;
      if (!loadProfile(uid)) {
        saveProfile(uid, {
          displayName: cred.user.displayName || cred.user.email.split('@')[0],
          email: cred.user.email, photoType: 'google',
          photoURL: cred.user.photoURL || '', createdAt: Date.now()
        });
      }
      DQAuth.closeModal();
    } catch (e) {
      if (e.code !== 'auth/popup-closed-by-user') showErr('Google sign-in failed. Try again.');
    }
  },
